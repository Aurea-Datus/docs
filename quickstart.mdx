---
title: "Quickstart"
description: "Begin using Datus Agent in minutes"
---
...
<Accordion icon="terminal" title="Start the CLI with Namespace">
    <Tip>
        <Tooltip tip="Configuration" cta="Read our Configuration guide" href="/configure">Configuration</Tooltip> You can connect to your database by adding a new namespace in <code>agent.yml</code>.
    </Tip>
    <CodeGroup>
        ```Bash Terminal
...
<Accordion icon="comments" title="Ask Datus Anything">
    <Tip>
    ...
    here is a table named `gold_vs_bitcoin`. Let’s view its schema.`
        ```Bash Terminal
...
<Accordion icon="rocket" title="Add a Table to the Current Context">
    <Note>
...
<Tip>For more command references and options, see <a href="/command">CLI</a>.<br /> Or simply type `.help`.</Tip>
...
<Accordion icon="docker" title="Run Datus Benchmark in Docker">
    <Tip>
...
<Accordion icon="play" title="Start with Environment Variables">
    <Tip>
    ...
    <Tabs>
        ...
        <Tab title="Spider-snow" >
            <CodeGroup>
                ```bash Spider 2.0-Snow
                    docker run --name datus \
                    --env DEEPSEEK_API_KEY=<your_api_key>  \
...
<Accordion icon="chart-column" title="Execute Built-in Benchmarks">
    <Warning>Each task may take several minutes to complete. Running all tasks may require hours or days depending on your system configuration.</Warning>

    <Accordion title="BIRD Dataset">
        <Info>Task ID range: 0-1533</Info>
        <Tabs>
            <Tab title="Run by Task ID">
                <CodeGroup>
                    ```bash BIRD
    ...
    <Accordion title="Spider 2.0-Snow Dataset">
        <Info>You can find the task ID (instance ID) in the [spider2-snow.jsonl](https://github.com/xlang-ai/Spider2/blob/main/spider2-snow/spider2-snow.jsonl) file.</Info>

        <Note>Ensure you start the Docker container with Snowflake environment parameters configured.</Note>
        <Tabs>
            <Tab title="Run by Task ID">
                <CodeGroup>
                    ```bash BIRD
